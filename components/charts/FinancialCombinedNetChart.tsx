import React, { useMemo } from "react";
import ReactECharts from "echarts-for-react";
import type { CotFinancialsRow } from "../../lib/types";

const LS = { color: "#64748b", fontFamily: "JetBrains Mono, monospace", fontSize: 10 };
const fmt = (v: number) => {
  const a = Math.abs(v), s = v < 0 ? "-" : "";
  return a >= 1e6 ? `${s}${(a / 1e6).toFixed(1)}M` : a >= 1e3 ? `${s}${(a / 1e3).toFixed(0)}K` : String(v);
};

const COLORS = {
  dealer: "#f59e0b",
  assetMgr: "#10b981",
  levMoney: "#0ea5e9",
  otherRept: "#a78bfa",
  nonrept: "#f43f5e",
};

const SERIES_NAMES = ["Dealer Net", "Asset Manager Net", "Leveraged Funds Net", "Other Reportables Net", "Nonreportable Net"] as const;
const COLOR_MAP: Record<string, string> = {
  "Dealer Net": COLORS.dealer,
  "Asset Manager Net": COLORS.assetMgr,
  "Leveraged Funds Net": COLORS.levMoney,
  "Other Reportables Net": COLORS.otherRept,
  "Nonreportable Net": COLORS.nonrept,
};

/** 5 net lines: Dealer, Asset Manager, Leveraged Funds, Other Reportables, Nonreportable. All visible by default, legend toggleable. */
export default function FinancialCombinedNetChart({ data }: { data: CotFinancialsRow[] }) {
  const option = useMemo(() => {
    const dates        = data.map((r) => r.as_of_date_in_form_yyyymmdd);
    const dealerNet    = data.map((r) => r.dealer_positions_long_all - r.dealer_positions_short_all);
    const assetMgrNet  = data.map((r) => r.asset_mgr_positions_long_all - r.asset_mgr_positions_short_all);
    const levMoneyNet  = data.map((r) => r.lev_money_positions_long_all - r.lev_money_positions_short_all);
    const otherReptNet = data.map((r) => r.other_rept_positions_long_all - r.other_rept_positions_short_all);
    const nonreptNet   = data.map((r) => r.nonrept_positions_long_all - r.nonrept_positions_short_all);

    return {
      backgroundColor: "transparent",
      legend: {
        data: [...SERIES_NAMES],
        selectedMode: true,
        textStyle: { color: "#94a3b8", fontFamily: "JetBrains Mono, monospace", fontSize: 10 },
        top: 0, right: 0, itemWidth: 14, itemHeight: 8,
      },
      tooltip: {
        trigger: "axis",
        backgroundColor: "#1e293b",
        borderColor: "#334155",
        textStyle: { color: "#e2e8f0", fontFamily: "JetBrains Mono, monospace", fontSize: 11 },
        formatter: (params: any[]) => {
          let html = `<div style="font-weight:600;margin-bottom:4px">${params[0]?.axisValue}</div>`;
          for (const p of params) {
            html += `<div style="display:flex;justify-content:space-between;gap:20px"><span style="color:#94a3b8">${p.seriesName}</span><span style="color:${COLOR_MAP[p.seriesName]};font-weight:600">${fmt(p.value)}</span></div>`;
          }
          return html;
        },
      },
      grid: { left: 60, right: 20, top: 36, bottom: 50 },
      xAxis: {
        type: "category", data: dates,
        axisLine: { lineStyle: { color: "#334155" } },
        axisLabel: { ...LS, rotate: 30, interval: Math.floor(dates.length / 8) },
        splitLine: { show: false },
      },
      yAxis: {
        type: "value", axisLine: { show: false }, axisTick: { show: false },
        axisLabel: { ...LS, formatter: fmt },
        splitLine: { lineStyle: { color: "#1e293b" } },
      },
      series: [
        {
          name: "Dealer Net", type: "line", data: dealerNet, smooth: 0.3, symbol: "none",
          lineStyle: { color: COLORS.dealer, width: 2 },
          markLine: {
            silent: true, symbol: "none",
            data: [{ yAxis: 0 }],
            lineStyle: { color: "#334155", type: "dashed", width: 1 },
            label: { show: false },
          },
        },
        { name: "Asset Manager Net", type: "line", data: assetMgrNet, smooth: 0.3, symbol: "none", lineStyle: { color: COLORS.assetMgr, width: 2 } },
        { name: "Leveraged Funds Net", type: "line", data: levMoneyNet, smooth: 0.3, symbol: "none", lineStyle: { color: COLORS.levMoney, width: 2 } },
        { name: "Other Reportables Net", type: "line", data: otherReptNet, smooth: 0.3, symbol: "none", lineStyle: { color: COLORS.otherRept, width: 2 } },
        { name: "Nonreportable Net", type: "line", data: nonreptNet, smooth: 0.3, symbol: "none", lineStyle: { color: COLORS.nonrept, width: 2 } },
      ],
    };
  }, [data]);
  return <ReactECharts option={option} style={{ height: "320px", width: "100%" }} opts={{ renderer: "canvas" }} />;
}
