...
export default function OtherReportablesChart({ data, start }: { data: CotFinancialsRow[]; start?: number }) {
  const option = useMemo(() => {
    const dates  = data.map((r) => r.as_of_date_in_form_yyyymmdd);
    const longs  = data.map((r) => r.other_rept_positions_long_all);
    const shorts = data.map((r) => r.other_rept_positions_short_all);
    const nets   = data.map((r) => r.other_rept_positions_long_all - r.other_rept_positions_short_all);
